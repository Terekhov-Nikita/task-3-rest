...
public class PromoActionService {
    ...
    public Long save( PromoData data){
        Promo promo = promoRepository.save(new Promo(0L,data.getName(),data.getDescription()));
        return promo.getId();
    }

    public void update(long id,  PromoData data){
        Promo promo = promoRepository.findById(id).orElseThrow(EntityNotFoundException::new);
    ...
    public PromoData get(long id){
        Promo action = promoRepository.findById(id).orElseThrow(EntityNotFoundException::new);
